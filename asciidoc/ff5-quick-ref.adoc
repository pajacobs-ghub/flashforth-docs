...
{\compileonly?!p>r?}  | Push contents of P to return stack and 
             | store new address to P. !( addr -- ) ( R: -- addr )! 
{\compileonly!r>p!}  | Pop from return stack to P register. !( R: addr -- )!  
...
!char! \textit{char} | Parse a character and leave ASCII value. !( -- n )! 
                          | For example: !char A!  !( -- 65 )! 
{\compileonly![char]!} \textit{char} | Compile inline ASCII character. !( -- )! 
\end{tabular}
...
\begin{tabular}{@{}ll@{}}
{\compileonly!s" !} \textit{text}!"! | Compile string into flash. !( -- )! 
                                 | At run time, leaves address and length. 
                                 | !( -- addr u )! 
{\compileonly!." !} \textit{text}!"! | Compile string to print into flash. !( -- )! 
...
!n=!  | Compare strings in RAM(!addr!) and Flash(!nfa!). 
           | Leave true if strings match, !n < 16!. 
           |  !( addr nfa u -- f )! 
!scan! | Scan string until !c! is found. 
            | !c-addr! must point to RAM and !u < 255!.
            | !( c-addr u c -- caddr1 u1 )! 
!skip! | Skip chars until !c! not found. 
            | !c-addr! must point to RAM and !u < 255!.
            | !( c-addr u c -- caddr1 u1 )! 
...
\begin{tabular}{@{}ll@{}}
{\compileonly!<#!}  | Begin conversion to formatted string. !( -- )! 
{\compileonly!#!}  | Convert 1 digit to formatted string. !( ud1 -- ud2 )! 
{\compileonly!#s!}  | Convert remaining digits. !( ud1 -- ud2 )! 
           | Note that !ud2! will be zero. 
{\compileonly!hold!}  | Append char to formatted string. !( c -- )! 
!sign!  | Add minus sign to formatted string, if !n<0!. !( n -- )! 
{\compileonly!#>!}  | End conversion, leave address and count 
...
\begin{tabular}{@{}ll@{}}
!cq:! \textit{name} | Create character queue. !( u -- )! 
...
To use many of the words listed in the following sections, load the text file !asm.txt!.
...
\begin{tabular}{@{}ll@{}}
!if,! \textit{xxx} !else,! \textit{yyy} !then,! | Conditional execution. !( cc -- )! 
!begin,! \textit{xxx} !again,! | Loop indefinitely. !( -- )! 
!begin,! \textit{xxx} \textit{cc} !until,! | Loop until condion is true. !( -- )! 
\end{tabular}
...
!ldd,! | Load indirect with dosplacement. !( Rd Rr q -- )! 
            | !Rr = {Y,Z}! 
...
Load them from a file with a name like !i2c-base-XXXX.txt! where !XXXX!
...
!i2c.ping?!  | Leaves !true! if the addressed slave device 
                  | acknowledges. !( 7-bit-addr -- f )!  
...
?i2c.c!?  | Send byte and leave !ack! bit. !( c -- ack )! 
               | Note that the !ack! bit will be high 
               | if the slave device did \textit{not} acknowledge. 
!i2c-addr-read!  | Address slave device for reading. 
                      | Leave !true! if slave acknowledged. 
                      | !( 7-bit-addr -- f )!  
!i2c.c@.ack!  | Fetch a byte and !ack! for another. 
                   | !( -- c )!  
...
!i2c.wait!  | Poll the I$^2$C hardware until the operation 
                 | has finished. !( -- )!  
!i2c.bus.reset!  | Clock through bits so that slave devices 
...
The following words are available as a common set of words for PIC18FXXK22, 
PIC24FV32KX30X and ATmega328P microcontrollers.
Load them from a file with a name like !spiN-base-XXXX.txt! where !XXXX!
...
!spi.wait!  | Poll the SPI peripheral until the operation 
                 | has finished. !( -- )! 
!spi.cexch! | Send byte !c1!, leave incoming byte !c2! on stack. 
                 | !( c1 -- c2 )! 
