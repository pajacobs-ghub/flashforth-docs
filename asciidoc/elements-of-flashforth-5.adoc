...
| _cond_ `if` _xxx_ `else` _yyy_ `then`
| Conditional execution.

| `begin` _xxx_ `again`
| Infinite loop.

| `begin` _xxx_ _cond_ `until`
| Loop until _cond_ is true.

| `begin` _xxx_ _cond_ `while` _yyy_ `repeat`
| Loop while _cond_ is true, _yyy_ is not executed on the last iteration.

| _u_ `for` _xxx_ `next` 
| Loop _u_ times.

| `endit` 
| Sets loop counter to zero so that we leave the loop when `next` is encountered.
